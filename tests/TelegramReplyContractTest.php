...
$testExtension = new erLhcoreClassExtensionLhctelegram();
$getSendMessageIdsMethod = new ReflectionMethod($testExtension, 'getTelegramSendMessageIds');
$getSendMessageIdsMethod->setAccessible(true);
if (class_exists('\\Longman\\TelegramBot\\Entities\\ServerResponse')) {
    $mockResponse = new \Longman\TelegramBot\Entities\ServerResponse([
        'ok' => true,
        'result' => [
            'message_id' => 182537,
            'date' => 1789114996,
            'chat' => ['id' => -1001946886605]
        ]
    ], 'contract_bot');
    $extractedIds = $getSendMessageIdsMethod->invoke($testExtension, $mockResponse);
